...
class Q4 extends Component {
  ...
  nahHandler = () => {
    ...
    setTimeout(() => {
      this.props.navigation.navigate('addPreference_q5', {
        preferences: {
  ...
  addAnotherHandler = () => {
    if (this.state.name.trim().length) {
      this.setState({
        q4: _.uniq([...this.state.q4, this.state.name.trim()]),
        name: '',
      });
    }
  };
  ...
  deleteNameHandler = name => {
    this.setState({
      q4: _.filter(this.state.q4, function(item) {
        return item !== name;
      }),
    });
  };
  ...
  activeButton = () => {
    return !!(this.state.q4.length || this.state.name.trim().length);
  };
