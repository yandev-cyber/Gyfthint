...
class Q2 extends Component {
  ...
  nahHandler = () => {
    ...
    setTimeout(() => {
      this.props.navigation.navigate('addPreference_q3', {
        preferences: {
  ...
  addAnotherHandler = () => {
    if (this.state.name.trim().length) {
      this.setState({
        q2: _.uniq([...this.state.q2, this.state.name.trim()]),
        name: '',
      });
    }
  };
  ...
  deleteNameHandler = name => {
    this.setState({
      q2: _.filter(this.state.q2, function(item) {
        return item !== name;
      }),
    });
  };
  ...
  activeButton = () => {
    return !!(this.state.q2.length || this.state.name.trim().length);
  };
          ...
          {this.state.showForm && (
            <View style={{alignItems: 'center', marginTop: hp('5%')}}>
              <Text
                ...
                What beer, wine, or liquor would you be excited
              </Text>
              <Text
                style={{
                  fontFamily: AppConstant.Fonts.normal,
